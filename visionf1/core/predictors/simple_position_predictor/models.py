    ...
    def load_model(self, name: str | None):
        ...
        for n, path in candidates:
            if os.path.exists(path):
                try:
                    # PATCH: Map 'app' to 'visionf1.core' for legacy pickles
                    import sys
                    import types
                    import visionf1.core
                    import visionf1.core.config

                    if "app" not in sys.modules:
                        app_module = types.ModuleType("app")
                        app_module.core = visionf1.core
                        app_module.config = visionf1.core.config
                        sys.modules["app"] = app_module
                        sys.modules["app.core"] = visionf1.core
                        sys.modules["app.config"] = visionf1.core.config
